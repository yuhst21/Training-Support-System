...
<html>
    <head>
        ...
        <script src="assets/js/pagger_user.js" type="text/javascript"></script>
                ...
                <div class="container-fluid">
                    <div class="layout-specing">
                        <div class="d-md-flex justify-content-between">
                            <h5 class="mb-0"><a href="subject_list">User List</a></h5>
                        ...
                        <!-- SEARCH BAR -->
                        <form action="users" role="search" method="get" id="searchform" class="searchform">
                            <input type="hidden" name="action" value="view"/>
                            <br/>
                            <!-- FILTER BAR -->
                            <select style="margin-right: 1px;margin-left: -3px;" class="s_filter select-1" name="filter_role">
                                <option value="-1">All Role</option>
                                <c:forEach items="${roles}" var="m">
                                    <option <c:if test="${m.setting_id == filter_role}">selected=""</c:if> 
                                                                                        value="${m.setting_id}">${m.setting_tiltle}</option>
                                </c:forEach>
                            </select>

                            <select style="margin-right: 13px;" class="s_filter select-1" name="filter_status">
                                <option value="-1">All Status</option>
                                <option <c:if test="${filter_status eq 1}">selected=""</c:if>  
                                                                           value="1">Active</option>
                                    <option <c:if test="${filter_status eq 0}">selected=""</c:if>  
                            ...
                            <button class="button-10" style="margin-left: 10px;" onclick="location.href = '#'" type="button">ADD</button>    
                                                ...
                                                <tr>
                                                    <td class="py-3 form-label">
                                                        <a href="#" class="text-dark">
                                                            <div class="d-flex align-items-center">
                                                                <c:if test="${u.avatar_url == null}"><img src="assets/images/user.png" class="avatar avatar-md-sm rounded-circle shadow" alt=""></c:if>
                                                                <c:if test="${u.avatar_url != null}"><img src="images/${u.avatar_url}" class="avatar avatar-md-sm rounded-circle shadow" alt=""></c:if>
                                                                <span class="ms-2 form-label">${u.full_name}</span>
                                                            </div>
                                                        </a>
                                                    </td>
                                                    <td class="p-3 form-label">${u.username}</td>
                                                    <td class="p-3 form-label">${u.email}</td>
                                                    <td class="p-3 form-label">${u.list.get(0).setting_tiltle}</td>
                                           
                                            <form id="status_form_${u.user_id}" action="users">
                                                <input type="hidden" name="action" value="view">
                                                <input name="page" type="hidden" value="${page_index}">
                                                <input name="txtsearch" type="hidden" value="${txtsearch}">
                                                <input type="hidden" name="change_status" value="${u.user_id}_${u.status}"/>
                                                <c:if test="${u.status eq true}">
                                                    <td class="p-3">
                                                        <span class="badge bg-soft-success">Active</span>
                                                        <label 
                                                            class="switch-wrap">
                                                            <input type="checkbox" class="open-model" id="status_check_${u.user_id}" checked onChange="modal_open(${u.user_id})"/>
                                                ...
                                                <c:if test="${u.status eq false}">
                                                    <td class="p-3">
                                                        ...
                                                        <label 
                                                            class="switch-wrap">
                                                            <input type="checkbox" id="status_check_${u.user_id}" onChange="modal_open(${u.user_id})"/>
                                                            <div class="switch"></div>
                                                        </label>
                                                    </td>
                                                </c:if>
                            ...
                            <div class="col-12 mt-4">
                                <div class="d-md-flex align-items-center text-center justify-content-between">
                                    <ul id="only-pagger" class="pagination justify-content-center mb-0 mt-3 mt-sm-0">
                                        <script>
                                            render("only-pagger",${page_index},${total_page}, 2, "${txtsearch}",${filter_role},${filter_status});
                                        </script>
                                    </ul>
                                </div>
                            </div><!--end col-->
