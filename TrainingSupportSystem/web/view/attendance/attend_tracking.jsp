            ...
            <main class="page-content bg-light">
                ...
                <div class="container-fluid">
                    <div class="layout-specing">
                        ...
                        <c:if test="${cs.size()==0}">
                            <p>You do not have any class to track attendance!</p> 
                        </c:if>
                        <c:if test="${cs.size()> 0}">
                                ...
                                <div class="p-4">
                                    <div class="table-responsive bg-white shadow rounded">
                                        <table class="table mb-0 table-center table-sortable">
                                            <thead>
                                                <tr>
                                                    <th  class="border-bottom p-3">Trainee Name</th>
                                                    <th  class="border-bottom p-3">UserName</th>
                                                    <th  class="border-bottom p-3">Absent(%)</th>
                                                        <c:forEach items="${schedule}" var="s">
                                                        <th class="border-bottom p-3">${s.toString()}<br/><br/> Slot ${schedule.indexOf(s)+1}</th>
                                                        </c:forEach>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <c:forEach items="${students}" var="s">
                                                    <tr>
                                                        ...
                                                        <td class="p-3 form-label">${s.getAbsent(class_id)}</td>

                                                        <c:forEach items="${schedule}" var="sche">
                                                            <c:if test="${s.getAttend(sche.schedule_id) eq null}">
                                                                <td class="p-3 form-label">
                                                                    -
                                                                </td>
                                                            </c:if>
                                                            <c:if test="${s.getAttend(sche.schedule_id) ne null}">
                                                                <td class="p-3 form-label">
                                                                    <c:if test="${s.getAttend(sche.schedule_id).status}">
                                                                        P
                                                                    </c:if>
                                                                    <c:if test="${!s.getAttend(sche.schedule_id).status}">
                                                                        A
